...
stim_train, stim_test = [
    random_pulses(p["dur_sim"], p["fs"], 8, 0.005, 0.04, 1, 5) for _ in range(2)
]
...
acc, reg = [np.zeros((n_reps, 2, len(dur_segment))) for _ in range(2)]
for i_r in range(n_reps):
    print(i_r)
    for i_a, alpha in enumerate([0, 2]):  # use white and pink noise
        noise = scale_noise(
            resp_train, power_law_noise(p["dur_sim"], p["fs"], alpha), snr_db
        )
        noise = np.expand_dims(noise, axis=1)
        for i_d, dur in enumerate(dur_segment):
            ...
            reg[i_r, i_a, i_d] = trf.regularization
            acc[i_r, i_a, i_d] = trf.predict(stim_test_segments, resp_test_segments)[-1]
